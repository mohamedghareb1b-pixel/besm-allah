    ...
    const cards = products.map(p => `
        <div class="product-card bg-white rounded-xl shadow-sm border flex flex-col overflow-hidden">
            ...
            <div class="p-2 flex flex-col flex-1 justify-between">
                ...
                <div class="mt-2">
                    <div class="flex items-center justify-between bg-gray-50 border rounded-lg px-2 py-1 mb-1.5">
                        <button onclick="decreasePreQty(${p.id})" class="qty-btn text-red-500 text-lg">−</button>
                        <span id="pre-qty-${p.id}" class="text-xs font-bold text-gray-700">1 ${p.unit||'كيلو'}</span>
                        <button onclick="increasePreQty(${p.id})" class="qty-btn text-green-600 text-lg">+</button>
                    </div>
...
function addToCartCard(productId) {
    ...
    if (!product) return;
    const qty = getPreQty(productId);
    const existing = cart.find(i => i.id === productId);
    if (existing) existing.qty += qty;
    else cart.push({ ...product, qty });
    // Reset pre qty
    preQty[productId] = 1;
    updatePreQtyDisplay(productId);
    updateCartUI();
    // Show added feedback
    showAddedFeedback(productId, qty, product.unit || 'كيلو');
}

function showAddedFeedback(productId, qty, unit) {
    // Flash the add button green briefly
    document.querySelectorAll(`[onclick="addToCartCard(${productId})"]`).forEach(btn => {
        const orig = btn.innerText;
        btn.innerText = `✅ تمت الإضافة (${qty} ${unit})`;
...
// ===== PRE QTY =====
const preQty = {}; // productId => qty before adding

function getPreQty(productId) {
    return preQty[productId] || 1;
}

function increasePreQty(productId) {
    preQty[productId] = getPreQty(productId) + 1;
    updatePreQtyDisplay(productId);
}

function decreasePreQty(productId) {
    preQty[productId] = Math.max(1, getPreQty(productId) - 1);
    updatePreQtyDisplay(productId);
}

function updatePreQtyDisplay(productId) {
    const p = DB.products.find(x => x.id === productId);
    const unit = p ? (p.unit || 'كيلو') : 'كيلو';
    document.querySelectorAll(`[id="pre-qty-${productId}"]`).forEach(el => {
        el.innerText = getPreQty(productId) + ' ' + unit;
    });
}
